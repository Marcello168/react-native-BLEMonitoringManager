 ...
 *  2541 透传 波特率类型
 */
typedef NS_ENUM (NSInteger, BaudRate_2541_Type) {
    BaudRate_2541_Type_9600bps = 0,
    BaudRate_2541_Type_19200bps,
    BaudRate_2541_Type_38400ps,
    BaudRate_2541_Type_57600ps,
    BaudRate_2541_Type_115200ps,
    BaudRate_2541_Type_2400bps,
    BaudRate_2541_Type_4800bps,
    BaudRate_2541_Type_14400bps,
    BaudRate_2541_Type_28800bps,
    BaudRate_2541_Type_76800bps,
    BaudRate_2541_Type_230400bps
};
 ...
 *  设定设备的波特率（2541）
 */
- (void)setDevice:(CBPeripheral *)peripheral baudRate:(BaudRate_2541_Type)type;
...
/**
 *  是否取消 OAD 升级,如果要将正在进行的 OAD 升级停止,将 canceled 设置为 YES
 */
...
/**
 *  切换到 OAD 升级模式后,使用startOADFileWithFileName: filePath:方法开始发送 OAD bin 文件的数据进行升级
 */
- (void)enter2541BigImageOADState:(CBPeripheral *)peripheal;

/**
 *  通过向指定的通道写入数据,然后进入 oad 模式后,然后,使用方法 startOADFileWithFileName: filePath: 开始升级,这两种模式的 oad 升级不需要选择 A 或者 B 版的 bin文件,这种模式的 oad 只有一个 bin 文件
 */
